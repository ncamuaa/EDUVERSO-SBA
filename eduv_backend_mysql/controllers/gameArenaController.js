...
const getLeaderboard = async (req, res) => {
  ...
  try {
    ...
    let whereClause = '';
    if (game_type) {
      whereClause = 'WHERE gs.game_type = $1';
      params.push(game_type);
    }

    const result = await db.query(
      `SELECT COALESCE(u.full_name, 'Unknown') AS player_name, gs.game_type, gs.high_score, gs.total_xp, gs.games_played
       FROM game_scores gs
       LEFT JOIN users u ON u.id = gs.user_id
       ${whereClause}
       ORDER BY gs.high_score DESC LIMIT 100`,
      params
    );
